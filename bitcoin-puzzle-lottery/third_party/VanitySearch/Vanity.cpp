...
VanitySearch::VanitySearch(Secp256K1 *secp, vector<std::string> &inputPrefixes,string seed,int searchMode,
  ...
  this->rekey = rekey;
  this->lotteryReseedEachKernel = lotteryReseedEachKernel;
  this->searchType = -1;
  ...
  if (lotteryReseedEachKernel) {
    printf("Puzzle lottery mode: fresh random GPU base keys every CUDA batch (maximum entropy)\n");
  } else if (rekey > 0) {
...
void VanitySearch::getCPUStartingKey(int thId,Int& key,Point& startP) {
  ...
  if (rekey > 0 || lotteryReseedEachKernel) {
    key.Rand(256);
  } else {
    key.Set(&startKey);
...
void VanitySearch::FindKeyGPU(TH_PARAM *ph) {
  ...
  // Global init
  int thId = ph->threadId;
  GPUEngine g(ph->gridSizeX,ph->gridSizeY, ph->gpuId, maxFound, (rekey!=0 || lotteryReseedEachKernel));
