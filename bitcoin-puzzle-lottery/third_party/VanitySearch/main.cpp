...
void printUsage() {
  ...
  printf("             [-nosse] [-r rekey] [-check] [-kp] [-sp startPubKey] [-lottery]\n");
  ...
  printf(" -lottery: Probabilistic mode — new random GPU base private keys every CUDA batch (implies rekey-style GPU path)\n");
  exit(0);
...
int main(int argc, char* argv[]) {
  ...
  bool lotteryReseedEachKernel = false;
  ...
  printf("GPU code not compiled, use -DWITHGPU when compiling.\n");
      ...
      a++;
    } else if (strcmp(argv[a], "-lottery") == 0) {
      lotteryReseedEachKernel = true;
      a++;
      printUsage();
    } else if (a == argc - 1) {
  ...
  VanitySearch *v = new VanitySearch(secp, prefix, seed, searchMode, gpuEnable, stop, outputFile, sse,
    maxFound, rekey, caseSensitive, startPuKey, paranoiacSeed, lotteryReseedEachKernel);
